...
def generate_audio_elevenlabs(text, title, api_key, voice_id):
    # عنوان URL الخاص بواجهة ElevenLabs مع استخدام voice_id المحدد
    url = f"https://api.elevenlabs.io/v1/text-to-speech/{voice_id}"
    headers = {
        "xi-api-key": api_key,
        "Content-Type": "application/json"
    }
    # إعداد الحمولة مع تضمين model_id لدعم اللغة العربية (النموذج متعدد اللغات)
    payload = {
        "text": text,
        "model_id": "eleven_multilingual_v2",  # هذا النموذج يدعم العربية
        "voice_settings": {
            "stability": 0.5,
...
# بيانات ElevenLabs (عرض خانة API فقط)
...
def generate_article(topic, language):
    ...
    if language == "العربية":
        prompt = (
            f"اكتب مقالاً طويلاً ومفصلاً ومميزًا عن '{topic}'. "
            "يجب أن يكون المقال شاملاً، يحوي تحليلاً عميقًا ورؤى فريدة، مع تقديم أمثلة وتفاصيل دقيقة حول جميع جوانب الموضوع. "
        ...
        prompt = (
            f"Write a long and detailed professional and exceptional article about '{topic}' that covers all aspects of the topic with in-depth analysis, unique insights, and detailed examples. "
...
def generate_and_save():
    ...
    if selected_output == "حفظ كملف وورد (DOCX)":
        ...
        if not api_key:
            ...
            return
        # استخدام الـ Voice ID الافتراضي
